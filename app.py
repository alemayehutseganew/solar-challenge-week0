...
#     """Download file from source and return as BytesIO object"""
...
#     """Load CSV file from source with caching"""
...
#     """Load notebook file from source with caching"""
...
#     status_text.text("📥 Loading data files from source...")
...
# def list_csvs():
#     """List available CSV files from source"""
...
# #     st.write("Available CSV files from source:", csv_files)
...
# notebook_name = nb_path.replace("gdrive:", "")
# st.markdown(f"**Notebook:** `{notebook_name}` (from source)")

# # Display available CSV files in sidebar
# if csv_files:
#     st.sidebar.subheader("Available Data Files (from source)")
...
#     with st.spinner('Loading data from source and executing notebook...'):
...
# st.info(f'''
# **Notes:** 
# - All notebooks and data files are loaded directly from source
...
@st.cache_data(ttl=3600, show_spinner="Downloading from source...")
def download_from_gdrive(file_id):
    """Download file from source and return as BytesIO object"""
    URL = "https://drive.google.com/uc?export=download&id="
